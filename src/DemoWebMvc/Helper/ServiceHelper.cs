...
namespace DemoWebMvc.Helper
{
    public class ServiceHelper : IServiceHelper
    {
        ...
        private ConcurrentBag<string> _projectServiceUrls;
        ...
        public ServiceHelper(IConfiguration configuration)
        {
            ...
            {
                //consul地址
                c.Address = new Uri(_configuration["ConsulAddress"]);
            });
        }

        public async Task<string> GetOrderAsync(string accessToken)
        {
            if (_projectServiceUrls == null)
        ...
        public async Task<string> GetProductAsync(string accessToken)
        {
            ...
            //每次随机访问一个服务实例
            var Client = new RestClient(_projectServiceUrls.ElementAt(new Random().Next(0, _projectServiceUrls.Count())));
        ...
        {
            var serviceNames = new string[] { "OrderService", "ProjectService" };
        ...
        private void GetServices(QueryOptions queryOptions, string serviceName)
        {
            ...
            if (queryOptions.WaitIndex != res.LastIndex)
            {
                ...
                else if (serviceName == "ProjectService")
                    _projectServiceUrls = new ConcurrentBag<string>(serviceUrls);
            }
        }
    }
}
